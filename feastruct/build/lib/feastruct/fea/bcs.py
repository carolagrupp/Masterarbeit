...
class BoundaryCondition:
    ...
    def __init__(self, val, dof, *node, **spring):
...
class NodalSpring(BoundaryCondition):
    """Class for a robin boundary condition acting at a node.

    Provides methods for the FEA solver and post-processing. 

    ! All plot functions to be still implemented !
    ...
    def __init__(self, node, val, dof):
        ...
        super().__init__(val, dof, node)

        # initialise the nodal reaction results
        self.reactions = []

    def apply_spring(self, K):
        """Applies the nodal support.

        The stiffness matrix is modified to apply the robin boundary by adding the spring 
        stiffness at the at the chosen degree of freedom to be equal to the
        specified value.
        ...
        K[gdof, gdof] = K[gdof, gdof] + self.val
...
class TwoNodalSpring(BoundaryCondition):
    """Class for a robin boundary condition acting at a node.
    ...
    def __init__(self, node1, node2, val, dof):
        """inits the NodalSupport class.
        ...
        # initialise the parent class
        super().__init__(val, dof, node1, node2)

        # initialise the nodal reaction results
        self.reactions = []

    def apply_twonodalspring(self,K):
        """Applies the spring betqeen two nodes.

        The stiffness matrix is modified to apply the robin boundary by adding the spring 
        stiffness at the chosen degree of freedom to be equal to the
        specified value.

        :param K: Global stiffness matrix of size *(N x N)*
        :type K: :class:`numpy.ndarray`
        """
        # get gdof number for first node
        gdof1 = self.node.dofs[self.dof].global_dof_num

        # get gdof number for second node
        gdof2 = self.node2.dofs[self.dof].global_dof_num


        # modify stiffness matrix
        K[gdof1, gdof2] = K[gdof1, gdof2] + self.val
        K[gdof2, gdof1] = K[gdof2, gdof1] + self.val
