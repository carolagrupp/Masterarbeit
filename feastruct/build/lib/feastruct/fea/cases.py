...
class FreedomCase(Case):
    ...
    def add_nodal_spring(self, node, val, dof):
        """Adds a nodal dirichlet boundary condition to the current freedom case.
        ...
        # add an entry to the freedom case items list
        new_spring = bcs.NodalSpring(node, val, dof)
        self.add_item(new_spring)

        return new_spring

    def add_twonodal_spring(self, node1, node2, val, dof):
        """Adds a nodal dirichlet boundary condition to the current freedom case.

        :param node1: The first node object at which the nodal support acts
        :param node2: The second node object at which the nodal support acts
        :type node: :class:`~feastruct.fea.node.Node`
        :param float val: The value of the nodal support - zero indicates a fixed support, whereas
            a non-zero value indicates a prescribed displacement
        :param int dof: The degree of freedom about which the boundary condition acts
        ...
        # add an entry to the freedom case items list
        new_twonodal_spring = bcs.TwoNodalSpring(node1, node2, val, dof)
        self.add_item(new_twonodal_spring)

        return new_twonodal_spring
