...
class LinearStatic(Solver):
    ...
    def solve(self):
        ...
        for (i, analysis_case) in enumerate(self.analysis_cases):
            ...
            K_mod= self.apply_spring(K=K_mod, analysis_case=analysis_case)

            # apply two nodal springs
            K_mod = self.apply_twonodalspring(K=K_mod, analysis_case=analysis_case)
            ...
            if self.solver_settings.linear_static.time_info:
                ...
                self.function_timer(str, self.calculate_reactions, K, u, f_eq, f_ext, analysis_case)
            else:
                self.calculate_reactions(K=K, u=u, f_eq=f_eq, f_ext=f_ext, analysis_case=analysis_case)
