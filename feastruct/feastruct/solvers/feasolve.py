...
from feastruct.fea.bcs import NodalSpring, NodalSupport, TwoNodalSpring
...
class Solver:
    ...
    def apply_bcs(self, K, f_ext, analysis_case):
        ...
        # # loop through all the supports in the analysis case
        # for spring in analysis_case.freedom_case.items:
        #     if isinstance(spring, NodalSpring):
        #         spring.apply_spring(K=K, f_ext=f_ext)

        return (K, f_ext)

    def apply_spring(self, K, analysis_case):
        """Applies the boundary conditions to the global stiffness matrix and external force
        vector for the analysis case.
        ...
        for spring in analysis_case.freedom_case.items:
            if isinstance(spring, NodalSpring):
                spring.apply_spring(K=K)

        return (K)
    
    def apply_twonodalspring(self, K, analysis_case):
        ...
        # loop through all the supports in the analysis case
        for spring in analysis_case.freedom_case.items:
            if isinstance(spring, TwoNodalSpring):
                spring.apply_twonodalspring(K=K)
    ...
    def calculate_reactions(self, K, u, f_eq, f_ext, analysis_case):
